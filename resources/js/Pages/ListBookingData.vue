            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="flex justify-between items-center gap-x-2 mb-4">
                    ...
                    <div>
                        ...
                        <div v-if="showInput" class="absolute w-full max-w-xs right-4 sm:right-56 mb-4 p-4 z-10 bg-slate-200 rounded-md shadow-lg">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-x-5 gap-y-4 my-2">
                    <div v-for="list in lists" :key="list.id" class="flex">
                        <Link :href="'list/' + list.id" class="w-full">
                        <div class="relative block justify-between shadow rounded-xl p-4 bg-gray-100 hover:shadow-lg hover:scale-105 transition duration-300 ease-in-out">
                            <Lucide class="w-5 h-5 absolute top-2 right-2 opacity-70 hover:opacity-100 transition duration-100 ease-in-out"
                                icon="Settings" @click.prevent.stop="openSetting(list)" 
                            />
            ...
            <template v-slot>
                <div class="p-6 bg-white rounded-lg">
                    ...
                    <div class="mb-4">
                        ...
                        <div id="asses" class="flex flex-col space-y-2">
                            ...
                            <label for="helper-radio-1" class="flex items-center cursor-pointer">
                                <input id="helper-radio-1" name="helper-radio" type="radio" value="private"
                                    v-model="privacyMode"
                                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500" />
                                <div class="ms-2">
                                    <span class="text-sm font-medium text-gray-900">List available only to you</span>
                                    <p id="helper-radio-text-1" class="text-xs font-normal text-gray-500">No one else
                                        can view this list</p>
                                </div>
                            </label>
                            <!-- Группа 2 -->
                            <label for="helper-radio-2" class="flex items-center cursor-pointer">
                                <input id="helper-radio-2" name="helper-radio" type="radio" value="link"
                                    v-model="privacyMode" @change="setAccessLink(editableList)"
                                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500" />
                                <div class="ms-2">
                                    <span class="text-sm font-medium text-gray-900">List available to everyone via a
                                        link</span>
                                    <p id="helper-radio-text-2" class="text-xs font-normal text-gray-500">Anyone with
                                        the link can view this list</p>
                                </div>
                            </label>
                            <!-- Группа 3 -->
                            <label for="helper-radio-3" class="flex items-center cursor-not-allowed text-gray-400">
                                <input id="helper-radio-3" name="helper-radio" type="radio" value="specific_users"
                                    v-model="privacyMode" disabled
                                ...
                                <div class="ms-2">
                                    ...
                                    <p id="helper-radio-text-3" class="text-xs font-normal">Only selected users can view
                                        this list</p>
                                </div>
                            </label>
                        </div>
                    ...
                    <div class="w-full">
                        <div v-if="privacyMode === 'link'" class="relative">
                            ...
                            <input id="npm-install-copy-text" type="text"
                                ...
                                :value="copyLink()" disabled readonly>
                            <button @click="copyLink(true)" data-copy-to-clipboard-target="npm-install-copy-text"
                                class="absolute end-2.5 top-1/2 -translate-y-1/2 text-gray-900 rounded-lg py-2 px-2.5 inline-flex items-center justify-center bg-white border-gray-200 border">
                                ...
                                <span v-else id="success-message" class="inline-flex items-center">
                                    <svg class="w-3 h-3 text-blue-700" aria-hidden="true"
                    ...
                    <div class="flex justify-between items-center mt-6">
                        <button @click="deleteList(editableList)"
                            class="w-12 flex justify-center gap-1 px-8 py-2 text-md font-medium hover:text-slate-100 hover:bg-red-600 rounded-lg transition duration-300 ease-in-out">Delete</button>
                        <button @click="updateList(editableList)"
                            class="w-12 flex justify-center gap-1 px-8 py-2 text-md font-medium hover:text-slate-100 hover:bg-slate-900 rounded-lg transition duration-300 ease-in-out">Save</button>
                    </div>
