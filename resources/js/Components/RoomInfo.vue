...
<template>
  <div>
    ...
    <div class="grid gap-4 mt-2 mb-4" :class="{
      ...
      <div v-for="room in rooms" :key="room.room_id">
        <div
          class="relative w-full shadow rounded-lg p-4 bg-gray-100 hover:shadow-lg hover:scale-105 transition duration-300 ease-in-out">
          <button v-if="props.listType === 'unit'" @click="removeFromList(room.room_id)"
    ...
    <AddToListModal v-if="props.lists" :lists="props.lists.unit" :itemId="selectedRoomId" type='unit' :auth="auth"
      :show="showAddToListModal" @close="closeAddToListModal" @updateLists="newList => {
